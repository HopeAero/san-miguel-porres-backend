...
import { UpdateInscriptionDto, UpdateCourseInscriptionDto } from '../../dto/update-inscription.dto';
...
export class UpdateInscriptionAction {
  ...
  @Transactional()
  async execute(id: number, updateInscriptionDto: UpdateInscriptionDto): Promise<Inscription> {
    ...
    const { studentId, schoolYearId, grade, courseInscriptions } = updateInscriptionDto;

    // Si se actualiza el estudiante, validar que existe
    if (studentId !== undefined && studentId !== existingInscription.studentId) {
    ...
    if (schoolYearId !== undefined && schoolYearId !== existingInscription.schoolYearId) {
    ...
    if ((studentId !== undefined && studentId !== existingInscription.studentId) || 
        (schoolYearId !== undefined && schoolYearId !== existingInscription.schoolYearId) || 
        (grade !== undefined && grade !== existingInscription.grade)) {
      
      const newStudentId = studentId === undefined ? existingInscription.studentId : studentId;
      const newSchoolYearId = schoolYearId === undefined ? existingInscription.schoolYearId : schoolYearId;
    ...
    await this.inscriptionRepository.update(id, {
      studentId: studentId === undefined ? existingInscription.studentId : studentId,
      schoolYearId: schoolYearId === undefined ? existingInscription.schoolYearId : schoolYearId,
    ...
    if (yearOrGradeChanged && (!courseInscriptions || courseInscriptions.length === 0)) {
      ...
      await this.recalculateCourseInscriptions(
        id, 
        schoolYearId === undefined ? existingInscription.schoolYearId : schoolYearId,
  ...
  private async updateCourseInscriptions(
    ...
    const existingCourseInscriptions = await this.courseInscriptionRepository.find({
    ...
    if (idsToRemove.length > 0) {
      await Promise.all(
        idsToRemove.map(id => this.courseInscriptionRepository.softDelete(id)),
      );
    }
